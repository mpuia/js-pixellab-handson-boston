...
console.warn(`
Folosind forEach, afiseaza numarul
...
din arrayul friends daca aceasta este mai mare sau egala cu 2 ani.
`);
person.friends.forEach(function (friend) {
...
`);
person.skills.forEach(function (skill) {
  if (!(skill.includes('j') || skill.includes('J'))) {
    console.log(skill);
  }
});

console.warn(`
Folosind forEach afiseaza propozitia: "Numele
mari ale prietenilor mei sunt xxx, xxx, xxx."
`);
...
console.warn(`
Folosind forEach, afiseaza suma anilor de
nastere a persoanelor
`);
...
console.warn(`
Folosind metoda reverse (+slice) si apoi
forEach, afiseaza in ordine inversa numele
complet al prietenilor din arrayul friends.
`);
person.friends.reverse().forEach(function (friend) {
  console.log(person.friends);
});
// cum integrez slice ca sa imi scoata varsta?
